...
namespace rpc {
  ...
  public:
    ...
    struct ApplyCustomDepthStencilValue : CommandBase<ApplyCustomDepthStencilValue> {
        ApplyCustomDepthStencilValue() = default;
        ApplyCustomDepthStencilValue(ActorId id, int value)
            : actor(id),
              custom_depth_stencil_value(value) {}
        ActorId actor;
        int custom_depth_stencil_value;
        MSGPACK_DEFINE_ARRAY(actor, custom_depth_stencil_value);
    };
