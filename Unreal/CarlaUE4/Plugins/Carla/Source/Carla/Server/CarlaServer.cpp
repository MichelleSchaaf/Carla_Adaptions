...
void FCarlaServer::FPimpl::BindActions()
{
  ...
  BIND_SYNC(set_actor_custom_depth_stencil_value) << [this](
      cr::ActorId ActorId,
      int bCustomDepthStencilValue) -> R<void>
  {
      REQUIRE_CARLA_EPISODE();
      FCarlaActor* CarlaActor = Episode->FindCarlaActor(ActorId);
      if (!CarlaActor)
      {
          return RespondError(
              "set_actor_custom_depth_stencil_value",
              ECarlaServerResponse::ActorNotFound,
              " Actor Id: " + FString::FromInt(ActorId));
      }

      CarlaActor->SetActorCustomDepthStencilValue(bCustomDepthStencilValue);
      return R<void>::Success();
  };
  // END NEW
...
#define MAKE_RESULT(operation) return parse_result(c.actor, operation);
  ...
  auto command_visitor = carla::Functional::MakeRecursiveOverload(
      ...
      [=](auto, const C::ApplyCustomDepthStencilValue& c) {MAKE_RESULT(set_actor_custom_depth_stencil_value(c.actor, c.custom_depth_stencil_value)); }, // NEW
